...
conn.ConnectionString = "Server = DESKTOP-Q867AF6\\SQLEXPRESS; Database = Users; User Id = woodbrom; Password = 112567; Trusted_Connection=True; Trust Server Certificate=true";
conn.Open();

// Create the command 
SqlCommand command = new SqlCommand("SELECT * FROM WoodbromTable", conn);
...
SqlCommand insertCommand = new SqlCommand("INSERT INTO WoodbromTable (UserId,UserName,  UserEmail) VALUES (@0, @1, @2)", conn);
...
try
{
    ...
    SqlCommand errorCommand = new SqlCommand("SELECT * FROM WoodbromTable", conn);
